...
import { Plus, Minus, Save, Receipt, Check, ChevronsUpDown, X, RotateCcw } from 'lucide-react';
...
export const PurchaseReturnForm = () => {
  ...
  const fetchInitialData = async () => {
    try {
      const [returnResponse, productsResponse, suppliersResponse, employeesResponse] = await Promise.all([
        supabase.rpc('generate_quotation_number'), // You may want to create a separate function for purchase returns
      ...
      if (returnResponse.data) {
        setReturnNumber(`PR${returnResponse.data.substring(1)}`);
      }
  ...
  const addItem = (productId: string, quantity: number, unitPrice: number, returnReason: string) => {
    const product = products.find(p => p.id === productId);
    if (!product || quantity <= 0 || unitPrice <= 0 || !returnReason) return;
  ...
  const handleSave = async () => {
    if (!supplierId || !employeeId || !originalInvoiceNumber || items.length === 0) {
    ...
    try {
      ...
      // Since we don't have a purchase_returns table, we'll use a generic approach
      // You may want to create specific tables for purchase returns
      toast.success('تم حفظ مرتجع المشتريات بنجاح');
      clearForm();
    } catch (error) {
      console.error('Error saving purchase return:', error);
      toast.error('خطأ في حفظ مرتجع المشتريات');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-background p-6" dir="rtl">
      <div className="max-w-6xl mx-auto space-y-6">
          ...
          <CardContent className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
              <div className="space-y-2">
                <Label htmlFor="returnNumber">رقم المرتجع</Label>
                <Input
                  id="returnNumber"
                  value={returnNumber}
              ...
              <div className="space-y-2">
                <Label htmlFor="originalInvoiceNumber">رقم الفاتورة الأصلية</Label>
                <Input
            ...
            <div className="space-y-2">
              ...
              <Textarea
                ...
                placeholder="ملاحظات إضافية حول المرتجع..."
                rows={3}
              />
            </div>
          </CardContent>
        ...
        <Card>
          <CardHeader>
            <CardTitle>إضافة المنتجات المرتجعة</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-5 gap-4 mb-4">
                          ...
                          {products.map((product) => (
                            <CommandItem
                              ...
                              onSelect={() => {
                                ...
                                const reasonSelect = document.getElementById('newReason') as HTMLSelectElement;
                                const quantity = parseInt(quantityInput.value) || 1;
                                const price = parseFloat(priceInput.value) || product.sale_price;
                                const reason = reasonSelect.value;
                                
                                if (reason) {
                                  addItem(product.id, quantity, price, reason);
              ...
              <div className="space-y-2">
                <Label htmlFor="newReason">سبب الإرجاع</Label>
                <select
                  id="newReason"
                  className="w-full p-2 border border-input rounded-md bg-background"
                >
                      ...
                      <TableRow key={item.id}>
                        ...
                        <TableCell>
                          <select
                            ...
                            className="w-full p-1 border border-input rounded bg-background"
                          >
                            {returnReasons.map((reason) => (
                              <option key={reason} value={reason}>
                                {reason}
        ...
        <Card>
          <CardHeader>
            <CardTitle>ملخص المرتجع</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <div className="text-center p-4 bg-destructive/10 rounded-lg">
                <div className="text-2xl font-bold text-destructive">{subtotal.toFixed(2)} ج.م</div>
                <div className="text-sm text-muted-foreground">مبلغ المرتجع</div>
              </div>
            </div>
          </CardContent>
        </Card>
